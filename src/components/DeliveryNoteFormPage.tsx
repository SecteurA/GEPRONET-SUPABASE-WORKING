...
const DeliveryNoteFormPage: React.FC<DeliveryNoteFormPageProps> = ({ onBack, onClientSelect }) => {
  ...
  const saveDeliveryNote = async () => {
    try {
      ...
      // Update delivery note with totals
      const deliveryNoteWithTotals = {
        ...deliveryNote,
      ...
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/save-delivery-note`, {
        ...
        body: JSON.stringify(deliveryNoteWithTotals),
      });
