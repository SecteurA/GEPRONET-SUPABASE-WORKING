...
const QuoteFormPage: React.FC<QuoteFormPageProps> = ({ onBack, preFilledData, onClientSelect }) => {
  ...
  const handleClientSelect = (client: any) => {
    ...
    if (client) {
      // Auto-fill quote fields with client data
      setQuote(prev => ({
        ...prev,
      ...
      // Clear quote fields when no client is selected
      setQuote(prev => ({
        ...prev,
        customer_name: '',
        customer_email: '',
  ...
  const saveQuote = async () => {
    try {
      ...
      // Set default valid until date if not provided (30 days from quote date)
      let validUntilDate = quote.valid_until_date;
      if (!validUntilDate) {
        const quoteDate = new Date(quote.quote_date);
        quoteDate.setDate(quoteDate.getDate() + 30);
        validUntilDate = quoteDate.toISOString().split('T')[0];
      }

      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/save-quote`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}`,
          'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        setError(result.error || 'Erreur lors de la sauvegarde du devis');
        return;
      }

      setSuccess(`Devis ${result.quote.quote_number} créé avec succès`);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setQuote(prev => ({ ...prev, valid_until_date: e.target.value }))}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#21522f] focus:border-transparent"
            />
            <p className="mt-1 text-sm text-gray-500">
              Si non spécifié, sera fixé à 30 jours
            </p>
          </div>
        </div>
      </div>
